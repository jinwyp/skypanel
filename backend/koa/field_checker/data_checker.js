...
let validation = {
    ...
    userIdInt: function(userId, field) {
        if (!validator.isInt(userId)) return throw409('user.userIdIntWrong', field)
    },

    userId: function(userId, field) {
        if (!validator.isMongoId(userId.toString())) return throw409('user.userIdWrong', field)
    },
    userAddressId: function(userAddressId, field) {
        if (!validator.isMongoId(userAddressId.toString())) return throw409('user.addressIdWrong', field)
    },

    username: function(username, field) {
        if (!username) return throw409('user.usernameRequired', field)
        if (!validator.isLength(username, 4, 60)) return throw409('user.usernameWrong', field)
    ...
    userPasswordMatch: function(password, confirmPassword, field) {
        if (!validator.isLength(password, 6, 30)) return throw409('user.passwordWrong', field)
        if (!validator.isLength(confirmPassword, 6, 30)) return throw409('user.passwordWrong', field)
        if (password !== confirmPassword) return throw409('user.passwordMatch', field)
    },
